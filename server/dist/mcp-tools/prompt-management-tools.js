...
export class PromptManagementTools {
    constructor(logger, mcpServer, configManager, onRefresh, onRestart) {
        this.promptsData = [];
     ...
     */
    ensureUserMessageTemplate(content, name, description) {
     ...
     */
    registerReadPrompt() {
        this.mcpServer.tool("read_prompt", "Read raw markdown content of a prompt. IMPORTANT: Call listprompts first to get valid prompt IDs. Use the exact ID shown in listprompts output (e.g. 'meeting_minutes', not 'meetingMinutes' or 'meeting-minutes').", {
            id: z.string().describe("Prompt ID or name from listprompts output"),
        }, { title: "Read Prompt", readOnlyHint: true }, async ({ id }, extra) => {
            try {
                ...
                for (const categoryImport of promptsConfig.imports || []) {
                    ...
                    if (!categoryJson.prompts || !Array.isArray(categoryJson.prompts))
                        continue;
                    const match = categoryJson.prompts.find((p) => p.id === id || p.name === id);
                    if (match) {
    ...
    registerCreatePrompt() {
        this.mcpServer.tool("create_prompt", "Create a new prompt in an existing category. IMPORTANT: Call listprompts first to see existing categories. Category must already exist — this tool does not auto-create categories. The `content` should include a `## User Message Template` section (Nunjucks-compatible); if omitted, the provided content is automatically wrapped as the template body.", {
            ...
            fullServerRestart: z
                .boolean()
                .optional()
                .describe("Whether to perform a full server restart after creating the prompt. Defaults to false (hot-reload only)."),
        }, { title: "Create Prompt" }, async (args, extra) => {
            try {
                ...
                if (args.fullServerRestart) {
                    setTimeout(() => this.onRestart(`Prompt created: ${args.name}`), 1000);
                    return {
                        content: [
                            {
                                type: "text",
                    ...
                    this.logger.info(`Hot-reload after creating prompt: ${args.name} completed.`);
                    return {
                        content: [
                            {
                                type: "text",
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Failed to create prompt: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
     ...
     */
    async createPromptImplementation(args) {
        const PROMPTS_FILE = this.configManager.getPromptsFilePath();
        const messages = [];
        const fileContent = await readFile(PROMPTS_FILE, "utf8");
        const promptsConfig = JSON.parse(fileContent);
        ...
        const categoryExists = promptsConfig.categories.some((cat) => cat.id === effectiveCategory);
        if (!categoryExists) {
            const availableCategories = promptsConfig.categories.map((cat) => cat.id).join(", ");
            throw new Error(`Category '${args.category}' does not exist. Available categories: ${availableCategories}`);
        }
        // Generate ID from name
        const id = args.name.toLowerCase().replace(/\s+/g, "_");
        // Check that prompt doesn't already exist
        const promptDirPath = path.join(path.dirname(PROMPTS_FILE), "prompts", effectiveCategory);
        ...
        if (existingPrompt) {
            throw new Error(`Prompt with ID '${id}' already exists in category '${effectiveCategory}'. Use 'update_prompt' to modify it.`);
        }
        ...
        const normalizedContent = this.ensureUserMessageTemplate(args.content, args.name, args.description);
    ...
    registerUpdatePrompt() {
        this.mcpServer.tool("update_prompt", "Update an existing prompt's content or description. IMPORTANT: Call listprompts first to get valid prompt names/IDs. To change category, use move_prompt instead.", {
            ...
            fullServerRestart: z
                .boolean()
                .optional()
                .describe("Whether to perform a full server restart after updating the prompt. Defaults to false (hot-reload only)."),
        }, { title: "Update Prompt" }, async (args, extra) => {
            try {
                this.logger.info(`Updating prompt: ${args.name}`);
                const result = await this.updatePromptImplementation(args);
                if (args.fullServerRestart) {
                    setTimeout(() => this.onRestart(`Prompt updated: ${args.name}`), 1000);
                    return {
                        content: [
                            {
                                type: "text",
                    ...
                    await this.onRefresh();
                    this.logger.info(`Hot-reload after updating prompt: ${args.name} completed.`);
                    return {
                        content: [
                            {
                                type: "text",
     ...
     */
    async updatePromptImplementation(args) {
        const PROMPTS_FILE = this.configManager.getPromptsFilePath();
        const promptsConfigDir = path.dirname(PROMPTS_FILE);
        const messages = [];
        const fileContent = await readFile(PROMPTS_FILE, "utf8");
        ...
        let foundCategoryPromptsPath = null;
        let foundCategoryPrompts = null;
        ...
        for (const categoryImport of promptsConfig.imports) {
            ...
            try {
                ...
                if (!categoryPromptsJson.prompts || !Array.isArray(categoryPromptsJson.prompts))
                    continue;
                const match = categoryPromptsJson.prompts.find((p) => p.name === args.name || p.id === expectedId);
                if (match) {
                    foundPrompt = match;
        ...
        if (!foundPrompt || !foundCategoryPromptsPath || !foundCategoryPrompts) {
            throw new Error(`Prompt '${args.name}' not found. Use 'create_prompt' to create a new prompt.`);
        }
        ...
        if (args.content) {
            ...
            const normalizedContent = this.ensureUserMessageTemplate(args.content, args.new_name || foundPrompt.name, args.description || foundPrompt.description);
            await safeWriteFile(fullPromptFilePath, normalizedContent, "utf8");
        ...
        if (args.description || args.new_name) {
            ...
            if (idx !== -1) {
                ...
                await safeWriteFile(foundCategoryPromptsPath, JSON.stringify(foundCategoryPrompts, null, 2), "utf8");
            }
        }
     ...
     */
    async createOrUpdatePromptFile(args, effectiveCategory, promptsFile) {
        const promptFilename = `${args.id}.md`;
        const promptDirPath = path.join(path.dirname(promptsFile), "prompts", effectiveCategory);
        const fullPromptFilePath = path.join(promptDirPath, promptFilename);
        ...
        const existingIndex = categoryPrompts.prompts.findIndex((p) => p.id === args.id);
     ...
     */
    registerDeletePrompt() {
        this.mcpServer.tool("delete_prompt", "Delete a prompt permanently. IMPORTANT: Call listprompts first to get valid prompt IDs. Use the exact ID shown in listprompts output (e.g. 'meeting_minutes', not 'meetingMinutes' or 'meeting-minutes').", {
            id: z.string().describe("Prompt ID or name from listprompts output"),
            fullServerRestart: z
                .boolean()
                .optional()
                .describe("Whether to perform a full server restart after deleting the prompt. Defaults to false (hot-reload only)."),
        }, { title: "Delete Prompt", destructiveHint: true }, async ({ id, fullServerRestart }, extra) => {
            try {
    ...
    async deletePromptImplementation(id) {
        ...
        try {
            ...
            this.logger.error(`Failed to read or parse promptsConfig.json at ${PROMPTS_CONFIG_FILE_PATH}`, e);
        ...
        let modifiedCategoryImportPath = null;
        let promptMarkdownFilePath = null;
        for (const categoryImport of [...promptsConfig.imports]) {
            const fullCategoryPromptsPath = path.join(promptsConfigDir, categoryImport);
            let categoryPromptsFileContent;
            let categoryPromptsJson;
            try {
                categoryPromptsFileContent = await readFile(fullCategoryPromptsPath, "utf8");
                categoryPromptsJson = JSON.parse(categoryPromptsFileContent);
            }
            catch (e) {
                this.logger.warn(`Could not read or parse category prompts file: ${fullCategoryPromptsPath}`, e);
                continue;
            }
            if (!categoryPromptsJson.prompts ||
                !Array.isArray(categoryPromptsJson.prompts)) {
                this.logger.warn(`Category file ${fullCategoryPromptsPath} has no valid 'prompts' array. Skipping.`);
                continue;
            }
            const promptToDeleteIndex = categoryPromptsJson.prompts.findIndex((p) => p.id === id || p.name === id);
            if (promptToDeleteIndex > -1) {
                const promptEntry = categoryPromptsJson.prompts[promptToDeleteIndex];
                promptMarkdownFilePath = path.join(path.dirname(fullCategoryPromptsPath), promptEntry.file);
                categoryPromptsJson.prompts.splice(promptToDeleteIndex, 1);
                await safeWriteFile(fullCategoryPromptsPath, JSON.stringify(categoryPromptsJson, null, 2), "utf8");
                messages.push(`✅ Removed prompt '${id}' from category file: ${categoryImport}`);
                ...
                if (promptMarkdownFilePath) {
                    try {
                        ...
                        if (unlinkError.code !== "ENOENT") {
                            messages.push(`⚠️ Could not delete markdown file '${promptEntry.file}': ${unlinkError.message}`);
                        }
                        else {
                            messages.push(`ℹ️ Markdown file '${promptEntry.file}' not found, possibly already deleted.`);
                        }
                    }
                }
                break;
        ...
        if (!promptFoundAndDeleted) {
            throw new Error(`Prompt with ID '${id}' not found in any category's prompts.json.`);
        }
        if (modifiedCategoryImportPath) {
            const fullModifiedCategoryPath = path.join(promptsConfigDir, modifiedCategoryImportPath);
            try {
                const categoryFileContent = await readFile(fullModifiedCategoryPath, "utf8");
                const categoryPromptsConfig = JSON.parse(categoryFileContent);
                if (categoryPromptsConfig.prompts &&
                    categoryPromptsConfig.prompts.length === 0) {
                    messages.push(`ℹ️ Category file '${modifiedCategoryImportPath}' is now empty.`);
                    promptsConfig.imports = promptsConfig.imports.filter((impPath) => impPath !== modifiedCategoryImportPath);
                    messages.push(`✅ Removed empty category import from promptsConfig.json.`);
                    const categoryIdMatch = modifiedCategoryImportPath.match(/(?:prompts[\\/])?([^\\/]+)[\\/]prompts\.json$/);
                    if (categoryIdMatch && categoryIdMatch[1]) {
                        ...
                        promptsConfig.categories = promptsConfig.categories.filter((cat) => cat.id !== categoryIdToRemove);
                        if (promptsConfig.categories.length < originalCategoryCount) {
                            messages.push(`✅ Removed category definition for '${categoryIdToRemove}' from promptsConfig.json.`);
                        }
                        ...
                        try {
                            ...
                            if (filesInDir.length === 1 && filesInDir[0] === "prompts.json") {
                                ...
                                messages.push(`✅ Deleted empty category directory: ${path.basename(categoryDirPath)}`);
                            }
                            else {
                                messages.push(`ℹ️ Category directory '${path.basename(categoryDirPath)}' was not empty, so it was not deleted.`);
                            }
                        }
                        catch (dirRemoveError) {
                            messages.push(`⚠️ Could not check or delete category directory '${path.basename(categoryDirPath)}': ${dirRemoveError.message}`);
                        }
                    }
                    else {
                        this.logger.warn(`Could not determine category ID from path '${modifiedCategoryImportPath}' to remove its definition.`);
                    }
                    await safeWriteFile(PROMPTS_CONFIG_FILE_PATH, JSON.stringify(promptsConfig, null, 2), "utf8");
                    this.logger.info("Updated promptsConfig.json after removing empty category references.");
                }
            }
            catch (readCheckError) {
                this.logger.error(`Could not re-read category file '${fullModifiedCategoryPath}' to check if empty:`, readCheckError);
            }
        }
        return {
            message: messages.join("\n"),
    ...
    registerCreateCategory() {
        this.mcpServer.tool("create_category", "Create a new empty category for organizing prompts.", {
            name: z.string().describe("Display name for the new category"),
            after: z
                .string()
                .optional()
                .describe("Category ID to insert the new category after. If omitted, appends to end."),
            fullServerRestart: z
                .boolean()
                .optional()
                .describe("Whether to perform a full server restart after creating. Defaults to false (hot-reload only)."),
        }, { title: "Create Category" }, async ({ name, after, fullServerRestart }, extra) => {
            try {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Failed to create category: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
     ...
     */
    async createCategoryImplementation(name, after) {
        const PROMPTS_FILE = this.configManager.getPromptsFilePath();
        const promptsConfigDir = path.dirname(PROMPTS_FILE);
        const messages = [];
        const fileContent = await readFile(PROMPTS_FILE, "utf8");
        ...
        const existing = promptsConfig.categories.find((cat) => cat.id === categoryId);
        if (existing) {
        ...
        if (after) {
            const afterIndex = promptsConfig.categories.findIndex((cat) => cat.id === after || cat.name.toLowerCase() === after.toLowerCase());
            if (afterIndex === -1) {
    ...
    registerDeleteCategory() {
        this.mcpServer.tool("delete_category", "Delete an empty category. IMPORTANT: Call listprompts first to verify the category exists and is empty. Category must have no prompts — move or delete prompts first.", {
            name: z.string().describe("Name or ID of the category from listprompts output"),
            fullServerRestart: z
                .boolean()
                .optional()
                .describe("Whether to perform a full server restart after deleting the category. Defaults to false (hot-reload only)."),
        }, { title: "Delete Category", destructiveHint: true }, async ({ name, fullServerRestart }, extra) => {
            try {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Failed to delete category: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
     ...
     */
    async deleteCategoryImplementation(name) {
        const PROMPTS_FILE = this.configManager.getPromptsFilePath();
        const promptsConfigDir = path.dirname(PROMPTS_FILE);
        const messages = [];
        const fileContent = await readFile(PROMPTS_FILE, "utf8");
        ...
        const categoryId = name.toLowerCase().replace(/\s+/g, "-");
        const categoryIndex = promptsConfig.categories.findIndex((cat) => cat.id === categoryId || cat.name.toLowerCase() === name.toLowerCase());
        if (categoryIndex === -1) {
            const availableCategories = promptsConfig.categories.map((cat) => cat.id).join(", ");
            throw new Error(`Category '${name}' not found. Available categories: ${availableCategories}`);
        }
        ...
        try {
            ...
            if (categoryPromptsJson.prompts && categoryPromptsJson.prompts.length > 0) {
                const promptNames = categoryPromptsJson.prompts
                    .map((p) => p.id)
                    .join(", ");
                throw new Error(`Category '${category.id}' is not empty. Delete or move these prompts first: ${promptNames}`);
            }
        }
        catch (error) {
        ...
        try {
            ...
            if (dirError.code !== "ENOENT") {
                messages.push(`⚠️ Could not delete category directory '${category.id}': ${dirError.message}`);
            }
        }
        return {
            message: messages.join("\n"),
    ...
    registerRenameCategory() {
        this.mcpServer.tool("rename_category", "Rename a category's display name. Only changes the name — ID, directory, and filenames stay the same.", {
            ...
            fullServerRestart: z
                .boolean()
                .optional()
                .describe("Whether to perform a full server restart after renaming. Defaults to false (hot-reload only)."),
        }, { title: "Rename Category" }, async ({ name, new_name, fullServerRestart }, extra) => {
            try {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Failed to rename category: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
     ...
     */
    async renameCategoryImplementation(name, newName) {
        const PROMPTS_FILE = this.configManager.getPromptsFilePath();
        const promptsConfigDir = path.dirname(PROMPTS_FILE);
        const fileContent = await readFile(PROMPTS_FILE, "utf8");
        const promptsConfig = JSON.parse(fileContent);
        ...
        const categoryIndex = promptsConfig.categories.findIndex((cat) => cat.id === lookupId || cat.name.toLowerCase() === name.toLowerCase());
        if (categoryIndex === -1) {
            const availableCategories = promptsConfig.categories.map((cat) => cat.id).join(", ");
            throw new Error(`Category '${name}' not found. Available categories: ${availableCategories}`);
        }
        // Reject if another category already has this display name
        const duplicate = promptsConfig.categories.find((cat, i) => i !== categoryIndex && cat.name.toLowerCase() === newName.toLowerCase());
        if (duplicate) {
            throw new Error(`A category named '${duplicate.name}' already exists (id: ${duplicate.id}).`);
        }
        const category = promptsConfig.categories[categoryIndex];
        ...
        if (newId !== oldId) {
            ...
            const idConflict = promptsConfig.categories.find((cat, i) => i !== categoryIndex && cat.id === newId);
            if (idConflict) {
                throw new Error(`A category with ID '${newId}' already exists (name: ${idConflict.name}).`);
            }
            ...
            try {
                ...
                if (catConfig.prompts && Array.isArray(catConfig.prompts)) {
                    ...
                    await safeWriteFile(categoryPromptsPath, JSON.stringify(catConfig, null, 2), "utf8");
                }
            }
            catch (err) {
                if (err.code !== "ENOENT")
     ...
     */
    registerMovePrompt() {
        this.mcpServer.tool("move_prompt", "Move a prompt from one category to another. IMPORTANT: Call listprompts first to get valid prompt IDs and category names. Both prompt and target category must exist.", {
            id: z.string().describe("Prompt ID or name from listprompts output"),
            target_category: z.string().describe("ID or name of the destination category from listprompts output"),
            fullServerRestart: z
                .boolean()
                .optional()
                .describe("Whether to perform a full server restart after moving the prompt. Defaults to false (hot-reload only)."),
        }, { title: "Move Prompt" }, async ({ id, target_category, fullServerRestart }, extra) => {
            try {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Failed to move prompt: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
     ...
     */
    async movePromptImplementation(id, targetCategory) {
        const PROMPTS_FILE = this.configManager.getPromptsFilePath();
        const promptsConfigDir = path.dirname(PROMPTS_FILE);
        const messages = [];
        const fileContent = await readFile(PROMPTS_FILE, "utf8");
        ...
        const targetCat = promptsConfig.categories.find((cat) => cat.id === targetCategoryId || cat.name.toLowerCase() === targetCategory.toLowerCase());
        if (!targetCat) {
            const available = promptsConfig.categories.map((cat) => cat.id).join(", ");
            throw new Error(`Target category '${targetCategory}' not found. Available categories: ${available}`);
        }
        // Find the prompt in its current category
        let sourceImportPath = null;
        let promptEntry = null;
        let sourceCategoryPromptsPath = null;
        for (const categoryImport of promptsConfig.imports) {
            const fullPath = path.join(promptsConfigDir, categoryImport);
            let categoryJson;
            try {
            ...
            if (!categoryJson.prompts || !Array.isArray(categoryJson.prompts))
                continue;
            const idx = categoryJson.prompts.findIndex((p) => p.id === id || p.name === id);
            if (idx > -1) {
     ...
     */
    registerReloadPrompts() {
        this.mcpServer.tool("reload_prompts", "Hot-reload all prompts from disk or perform a full server restart. Use after creating, updating, or deleting prompts to refresh the server state.", {
            fullServerRestart: z
                .boolean()
                .optional()
                .describe("Whether to perform a full server restart after reloading prompts. Defaults to false (hot-reload only)."),
            reason: z
                .string()
                .optional()
                .describe("Optional reason for reloading/restarting"),
        }, { title: "Reload Prompts" }, async ({ fullServerRestart, reason, }, extra) => {
            const reloadReason = reason || "Manual reload requested";
            this.logger.info(`Reload prompts request received${fullServerRestart ? " with restart" : ""}: ${reloadReason}`);
            try {
                if (fullServerRestart) {
...
//# sourceMappingURL=prompt-management-tools.js.map