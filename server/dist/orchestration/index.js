...
import { createTransportManager, TransportManager, } from "../transport/index.js";
...
export class ApplicationOrchestrator {
    ...
    async startup() {
        try {
            ...
            this.logger.info("Application orchestrator startup completed successfully");
        }
        catch (error) {
            if (this.logger) {
    ...
    async determineServerRoot() {
        ...
        for (let i = 0; i < strategies.length; i++) {
            ...
            try {
                ...
                if (i === lastHighPriorityIndex &&
                    !isVerbose &&
                    lastHighPriorityIndex >= 0) {
                    if (!isQuiet) {
                        console.error(`⚠️  High-priority detection strategies failed. Trying fallback methods...`);
                        console.error(`💡 Tip: Set MCP_SERVER_ROOT environment variable for instant detection`);
...
`;
        ...
        throw new Error(`Unable to determine server root directory after testing ${strategies.length} strategies.\n\n` +
    ...
    async initializeFoundation() {
        ...
        const args = process.argv.slice(2);
        const transport = TransportManager.determineTransport(args, this.configManager);
    ...
    async loadAndProcessData() {
        ...
        this.promptManager = new PromptManager(this.logger, this.configManager, this.mcpServer);
        ...
        if (process.env.MCP_PROMPTS_CONFIG_PATH) {
            ...
            if (isVerbose) {
                this.logger.info("🎯 Using MCP_PROMPTS_CONFIG_PATH environment variable override");
            }
        }
        else {
        ...
        if (isVerbose) {
            ...
            if (process.env.MCP_PROMPTS_CONFIG_PATH) {
                this.logger.info(`🎯 MCP_PROMPTS_CONFIG_PATH override: "${process.env.MCP_PROMPTS_CONFIG_PATH}"`);
            }
            else {
                this.logger.info(`Config manager base directory: "${path.dirname(this.configManager.getPromptsFilePath())}"`);
            }
            ...
            this.logger.info(`__filename equivalent: ${fileURLToPath(import.meta.url)}`);
            this.logger.info(`Config file path: ${this.configManager.configPath}`);
            this.logger.info(`MCP_PROMPTS_CONFIG_PATH: ${process.env.MCP_PROMPTS_CONFIG_PATH || "undefined"}`);
            this.logger.info(`MCP_SERVER_ROOT: ${process.env.MCP_SERVER_ROOT || "undefined"}`);
            this.logger.info(`PROMPTS_FILE is absolute: ${path.isAbsolute(PROMPTS_FILE)}`);
            this.logger.info(`PROMPTS_FILE normalized: ${path.normalize(PROMPTS_FILE)}`);
        }
        // Validate that we're using absolute paths (critical for Claude Desktop)
        if (!path.isAbsolute(PROMPTS_FILE)) {
            if (isVerbose) {
                this.logger.error(`⚠️  CRITICAL: PROMPTS_FILE is not absolute: ${PROMPTS_FILE}`);
                this.logger.error(`This will cause issues with Claude Desktop execution!`);
            }
            ...
            if (isVerbose) {
                this.logger.info(`🔧 Converting to absolute path: ${absolutePromptsFile}`);
            }
        ...
        try {
            ...
            this.logger.error(`✗ Prompts configuration file NOT FOUND: ${PROMPTS_FILE}`);
            if (isVerbose) {
        ...
        try {
            ...
            const result = await this.promptManager.loadAndConvertPrompts(PROMPTS_FILE, path.dirname(PROMPTS_FILE));
            ...
            this.logger.info(`✓ Loaded ${this.promptsData.length} prompts from ${this.categories.length} categories`);
            this.logger.info(`✓ Converted ${this.convertedPrompts.length} prompts to MCP format`);
            // Log category breakdown
            if (this.categories.length > 0) {
                this.logger.info("Categories loaded:");
                this.categories.forEach((category) => {
                    const categoryPrompts = this.promptsData.filter((p) => p.category === category.id);
            ...
            if (this.mcpToolsManager) {
                this.mcpToolsManager.updateData(this.promptsData, this.convertedPrompts, this.categories);
            }
            if (this.apiManager) {
                // apiManager might not exist for stdio
                this.apiManager.updateData(this.promptsData, this.categories, this.convertedPrompts);
            }
            // CRUCIAL STEP: Re-register all prompts with the McpServer using the newly loaded data
            if (this.promptManager && this.mcpServer) {
                this.logger.info("🔄 Re-registering all prompts with MCP server after hot-reload...");
                const registeredCount = await this.promptManager.registerAllPrompts(this.convertedPrompts);
                this.logger.info(`✅ Successfully re-registered ${registeredCount} prompts.`);
            }
            else {
                this.logger.warn("⚠️ PromptManager or McpServer not available, skipping re-registration of prompts after hot-reload.");
            }
        }
        catch (error) {
    ...
    async initializeModules() {
        ...
        this.mcpToolsManager = createMcpToolsManager(this.logger, this.mcpServer, this.promptManager, this.configManager, () => this.fullServerRefresh(), (reason) => this.restartServer(reason));
    ...
    async startServer() {
        ...
        this.transportManager = createTransportManager(this.logger, this.configManager, this.mcpServer, transport);
        const viewerConfig = this.configManager.getViewerConfig();
        const needsApiManager = this.transportManager.isSse() || viewerConfig?.autoStart;
        if (needsApiManager) {
            this.apiManager = createApiManager(this.logger, this.configManager, this.promptManager, this.mcpToolsManager);
            // Update API manager with current data
            this.apiManager.updateData(this.promptsData, this.categories, this.convertedPrompts);
        }
        // Start the server
        this.serverManager = await startMcpServer(this.logger, this.configManager, this.transportManager, this.apiManager);
    ...
    async fullServerRefresh() {
        this.logger.info("🔥 Orchestrator: Starting full server refresh (hot-reload)...");
        try {
 ...
 */
export function createApplicationOrchestrator() {
