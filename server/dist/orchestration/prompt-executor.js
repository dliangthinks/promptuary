 ...
 */
import { PromptError, ValidationError, handleError as utilsHandleError, } from "../utils/errorHandling.js";
/**
 * Prompt Executor class
 */
export class PromptExecutor {
    constructor(logger, promptManager, conversationManager) {
     ...
     */
    updatePrompts(convertedPrompts) {
     ...
     */
    async runPromptDirectly(promptId, parsedArgs) {
        try {
            const convertedPrompt = this.convertedPrompts.find((cp) => cp.id === promptId);
            if (!convertedPrompt) {
                throw new PromptError(`Could not find prompt with ID: ${promptId}`);
            }
            this.logger.debug(`Running prompt directly: ${promptId} with arguments:`, parsedArgs);
            ...
            userMessageText = await this.promptManager.processTemplateAsync(userMessageText, parsedArgs, specialContext, convertedPrompt.tools || false);
            ...
            const { message } = this.handleError(error, `Error executing prompt '${promptId}'`);
            return message;
        }
    }
    /**
    ...
    async executePromptChain(chainPromptId, inputArgs = {}) {
        try {
            ...
            this.logger.info(`Executing prompt chain: ${chainPrompt.name} (${chainPrompt.id}) with ${totalSteps} steps`);
            ...
            for (let i = 0; i < chainPrompt.chainSteps.length; i++) {
                ...
                this.logger.info(`Executing chain step ${currentStepNumber}/${totalSteps}: ${step.stepName} (${step.promptId})`);
                ...
                if (step.inputMapping) {
                    for (const [stepInput, resultKey] of Object.entries(step.inputMapping)) {
                        if (results[resultKey] !== undefined) {
                            ...
                            this.logger.warn(`Missing input mapping for step '${step.stepName}': ${resultKey} -> ${stepInput}`);
                        }
                    }
                }
                ...
                try {
                    ...
                    const stepResult = await this.runPromptDirectly(step.promptId, stepArgs);
                    // Store the result
                    ...
                    const { message, isError } = this.handleError(stepError, `Error executing chain step '${step.stepName}'`);
            ...
            this.logger.info(`Chain execution completed successfully: ${chainPrompt.id}`);
            return {
                results,
                messages,
            };
            ...
            const { message } = this.handleError(error, `Error executing prompt chain '${chainPromptId}'`);
            return {
 ...
 */
export function createPromptExecutor(logger, promptManager, conversationManager) {
