...
export class PromptManager {
    constructor(logger, configManager, mcpServer) {
        this.logger = logger;
        ...
        if (mcpServer) {
            this.registry = new PromptRegistry(logger, mcpServer, configManager, this.templateProcessor);
        }
    }
    /**
    ...
    async processTemplateAsync(template, args, specialContext = {}, toolsEnabled = false) {
        return this.templateProcessor.processTemplateAsync(template, args, specialContext, toolsEnabled);
    }
    /**
     ...
     */
    async loadAndConvertPrompts(configPath, basePath) {
        try {
            ...
            this.logger.info(`📊 Raw data loaded: ${promptsData.length} prompts from ${categories.length} categories`);
            ...
            this.logger.info("🎉 PromptManager.loadAndConvertPrompts() completed successfully");
