...
export class PromptRegistry {
    constructor(logger, mcpServer, configManager, templateProcessor) {
    ...
    async registerAllPrompts(prompts) {
        try {
            this.logger.info(`Registering ${prompts.length} prompts with the server...`);
            ...
            for (const promptData of prompts) {
                try {
                    const success = await this.registerSinglePrompt(promptData, registrationMode);
                    if (success) {
                        registeredCount++;
                    }
                }
                catch (error) {
                    this.logger.error(`Error registering prompt '${promptData.name}':`, error);
                    // Continue with other prompts even if one fails
                }
            }
            this.logger.info(`Successfully registered ${registeredCount} prompts with the server`);
     ...
     */
    async registerSinglePrompt(promptData, registrationMode = "both") {
        try {
            ...
            this.logger.error(`Error registering single prompt '${promptData.name}':`, error);
            return false;
        }
    }
    /**
    ...
    async unregisterAllPrompts() {
        if (this.registeredPromptHandles.length > 0) {
            try {
                ...
                this.logger.warn("Could not unregister existing prompts:", unregisterError);
                this.registeredPromptHandles = [];
            }
        }
    }
     ...
     */
    createPromptHandler(promptData) {
            ...
            try {
                ...
                if (effectivelyEmptyArgs &&
                    ...
                    if (promptData.arguments && promptData.arguments.length > 0) {
                        ...
                        responseText += `\nExample usage: >>${promptData.id || promptData.name} ${promptData.arguments
                ...
                userMessageText = await this.templateProcessor.processTemplateAsync(userMessageText, args, { previous_message: this.getPreviousMessage() }, promptData.tools || false);
    ...
    addToConversationHistory(item) {
        ...
        if (this.conversationHistory.length > this.MAX_HISTORY_SIZE) {
            ...
            this.conversationHistory.splice(0, this.conversationHistory.length - this.MAX_HISTORY_SIZE);
            this.logger.debug(`Trimmed conversation history to ${this.MAX_HISTORY_SIZE} entries to prevent memory leaks`);
        }
    }
    /**
    ...
    async executePromptDirectly(promptId, args, prompts) {
        try {
            ...
            const userMessageText = await this.templateProcessor.processTemplateAsync(convertedPrompt.userMessageTemplate, args, { previous_message: this.getPreviousMessage() }, convertedPrompt.tools || false);
