...
export class SemanticAnalyzer {
     ...
     */
    analyzeContent(userTemplate, systemMessage) {
        const reasoning = [];
        const suggestedGates = [];
        let executionType = "template";
        let requiresExecution = false;
     ...
     */
    combineAnalyses(contentAnalysis, structuralAnalysis) {
        // Combine confidence scores
        const combinedConfidence = Math.min(1.0, (contentAnalysis.confidence || 0) + (structuralAnalysis.confidence || 0));
        // Determine final execution type
        let finalExecutionType = contentAnalysis.executionType || "template";
        // Determine if execution is required
        const finalRequiresExecution = (contentAnalysis.requiresExecution || false) ||
     ...
     */
    getAnalysisSummary(classification) {
