...
export function registerMcpApps(mcpServer, logger) {
    const resourceUri = "ui://promptuary/index.html";
    const appDistDir = getAppDistDir();
    // Register the App tool — entry point for opening the prompt manager UI
    registerAppTool(mcpServer, "prompt_manager", {
        title: "Prompt Manager",
        description: "Opens the Promptuary prompt management UI. Browse, execute, edit, and delete prompts via an interactive icon grid interface.",
        inputSchema: {},
    ...
    registerAppResource(mcpServer, "Promptuary App", resourceUri, {
        ...
        try {
            ...
            logger.warn(`Failed to read app bundle at ${htmlPath}: ${err instanceof Error ? err.message : String(err)}`);
            return {
                contents: [
                    {
                        uri: resourceUri,
